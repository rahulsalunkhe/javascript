...
var cakeType = "Vanilla" //Defines the type of cake 

if(cakeType == "Chocolate" || cakeType == "Vanilla") //First level decision to check if cake is either chocolate or vanilla
{
	if(cakeType != "Chocolate") //Second level check to decide if cake is Chocolate or not
	{
		console.log("Cake is a Vanilla one"); //True block states that the cake is vanilla one
	}
	else
	{
		console.log("Cake is a Chocolate one"); //False block states that the cake is chocolate one
	}
}